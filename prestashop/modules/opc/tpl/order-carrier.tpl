...
{if !$opc}<h1>{l s='Shipping'}</h1>{else}<p class="onestepcheckout-numbers onestepcheckout-numbers-2"> {l s='Delivery methods'}</p>{/if}
							...
							<tr>
								<td class="delivery_option_logo">
									{foreach $option.carrier_list as $carrier}
										{if $carrier.logo}
											<img src="{$carrier.logo}" alt="{$carrier.instance->name}" width="45px" />
