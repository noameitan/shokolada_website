					...
					{foreach from=$banners key='index' item='banner' name='banner'}
						<tr name="banner_{$banner.id_blockadvertmulti}" id="bannerTable{$block}_{$banner.id_blockadvertmulti}" {if $smarty.foreach.banner.index % 2}class="alt_row"{/if}>
							...
							<td {if $leftBanners >= 2}class="dragHandle"{/if} id="td_bannerTable{$block}_{$banner.id_blockadvertmulti}" width="40">
								<a {if $smarty.foreach.banner.first}style="display: none;"{/if} href="{$currentIndex}&id_banner={$banner.id_blockadvertmulti}&direction=0&changePosition={$rand}#{$banner.block_id}"><img src="../img/admin/up.gif" alt="{l s='Up' mod='blockadvertmulti'}" title="{l s='Up' mod='blockadvertmulti'}" /></a><br />
								<a {if $smarty.foreach.banner.last}style="display: none;"{/if} href="{$currentIndex}&id_banner={$banner.id_blockadvertmulti}&direction=1&changePosition={$rand}#{$banner.block_id}"><img src="../img/admin/down.gif" alt="{l s='Down' mod='blockadvertmulti'}" title="{l s='Down' mod='blockadvertmulti'}" /></a>
							</td>
							<td><a class="vergrande" rel="bannerTable{$block}" href="{$img_path}{$banner.image_name}" >
								<img src="{$img_path}{$banner.image_name}" name="image_{$banner.id_blockadvertmulti}" style="width:80px;height:80" /><input type="hidden" name="image_name_{$banner.id_blockadvertmulti}" value="{$banner.image_name}"/></a></td>
