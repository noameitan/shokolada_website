...
class MagicZoom extends Module {
    ...
    private function sendStat($action = '') {
        ...
        if('working' == 'v5.4.32' || 'working' == 'v4.5.14') {
            return;
        }
        ...
        $path = "api/stat/?action={$action}&tool_name=magiczoom&license=trial&tool_version=v4.5.14&module_version=v5.4.32&platform_name=prestashop&platform_version={$platformVersion}&url={$url}";
                    ...
                    foreach($groups as $group) {
                        foreach($group as $param) {
                            if(isset($_POST[$blockId.'-'.$param])) {
                                $valueToSave = $value = trim($_POST[$blockId.'-'.$param]);
    ...
    public function parseTemplateStandard($output, $smarty) {
        ...
        switch($currentTemplate) {
            ...
            case 'product':
                ...
                $cover = $smarty->$getTemplateVars('cover');
                    ...
                    if(count($images) && !$GLOBALS['magictoolbox']['magic360']['class']->params->checkValue('enable-effect', 'No', 'product')) {
                        ...
                        $image = 
                            '<div style="position: relative;">'.
                                '<div id="mainImageContainer" style="position: absolute; left: -1000px;">'.
            ...
            case 'blockspecials':
                ...
                $image = '<div class="MagicToolboxContainer">'.$image.'</div>';
                $pattern = '<img[^>]*?src="[^"]*?'.preg_quote($_link->getImageLink($lrw, $product['id_image'], 'medium'.$this->imageTypeSuffix), '/').'"[^>]*>';
            ...
            case 'blockviewed':
                ...
                foreach($productsViewed as $id_product) {
                    ...
                    $pattern = '<img[^>]*?src="[^"]*?'.preg_quote($_link->getImageLink($lrw, $productViewedObj->cover, 'medium'.$this->imageTypeSuffix), '/').'"[^>]*>';
                    $pattern = '(<a[^>]*?href="[^"]*?"[^>]*>[^<]*)?'.$pattern.'([^<]*<\/a>)?';
                    $output = preg_replace('/'.$pattern.'/is', $image, $output);
                }
                break;
            ...
            case 'blocknewproducts':
                if(isset($blockbestsellers)) {
                    ...
                    $products = ProductSale::getBestSalesLight(intval($cookie->id_lang), 0, version_compare(_PS_VERSION_, '1.5.1.0', '>=') ? 5 : 4);
                } else {
                ...
                if($pCount) {
                    ...
                    for($i = 0; /*$i < 2 &&*/ $i < $pCount; $i++) {
                        ...
                        $image = '<div class="MagicToolboxContainer">'.$image.'</div>';
                        if($this->isPrestahop15x && isset($blockbestsellers)) {
                            $pattern = '<img[^>]*?src="[^"]*?'.preg_quote($_link->getImageLink($lrw, $products[$i]['id_image'], 'small'.$this->imageTypeSuffix), '/').'"[^>]*>';
                        } else {
                            $pattern = '<img[^>]*?src="[^"]*?'.preg_quote($_link->getImageLink($lrw, $products[$i]['id_image'], 'medium'.$this->imageTypeSuffix), '/').'"[^>]*>';
                        }
